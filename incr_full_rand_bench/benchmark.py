...
def increment_benchmark_settings(rows:int, cols:int, n_simulations:int, n_components:int, solver:str) -> float:
    ...
    for i in range(n_simulations):
        pca = PCA(n_components=int(n_components), svd_solver=solver, random_state=1, iterated_power=7)
        s = time.time()
        pca.fit(X)
        e = time.time()
    ...
    with open(f'prem_data_slow_sm/{rows}_{cols}_{solver}_{n_components}.txt', 'w') as file:
    ...
    while i <= 105000:
        ...
        for comp in components:
            ...
            for k in cols_p:
                print(i,k)
                for s in solvers:
                    increment_benchmark_settings(i,k,1,comp,s)
