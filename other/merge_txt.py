...
def read_csv(name):
    ...
    columns = ['solver','r','c','comp','time','sing_values']
    # columns = ['row','solver','r','c','time','comp','density','sing_values']
...
def calculate_norm1(row):
    ...
    if np.linalg.norm(sing_values_df2) == 0:
        return np.nan
    return np.linalg.norm(sing_values_df2 - sing_values) / np.linalg.norm(sing_values_df2)*100
...
def calculate_norm2(row):
    ...
    if np.linalg.norm(sing_values_df2) == 0:
        return np.nan
    return np.linalg.norm((sing_values_df2 - sing_values)/sing_values, ord=2)*100
